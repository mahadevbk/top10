...
def scrape_rotten_tomatoes(category):
    url_map = {
        "Thrillers": "https://www.rottentomatoes.com/browse/movies_at_home/genres:thriller",
        ...
        "Sci-Fi": "https://www.rottentomatoes.com/browse/movies_at_home/genres:sci-fi"
    }
    ...
    try:
        ...
        items = soup.find_all("a", class_="js-tile-link")[:10]  # Adjust class based on site structure
        results = []
        for item in items:
            title = item.find("span", class_="p--small").text.strip() if item.find("span", class_="p--small") else "N/A"
            rating = item.find("score-pairs")["criticsscore"] if item.find("score-pairs") else "N/A"
...
def scrape_imdb(category):
    ...
    try:
        ...
        response.raise_for_status()
        soup = BeautifulSoup(response.text, "html.parser")
        items = soup.find_all("div", class_="sc-b189961a-0 iqHBGn")[:10]  # Adjust class based on IMDb structure
        results = []
        for item in items:
